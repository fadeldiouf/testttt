...
const  modulesURL = '/v1/diplomes';
const encours = modulesURL+'/encours';
const retirer = modulesURL+'/retirer';
const moveToAtraiter = modulesURL+'/moveToAtraiter';
const moveToDisponible = modulesURL+'/moveToDisponible';
const disponible = modulesURL+'/disponible';
const atraiter = modulesURL+'/atraiter';
export const useDiplomeStore = defineStore('diplome', {
  ...
  actions: {
    ...
    async encoursPayer() {
      ...
      this.error = '';
      try {
        const response = await axios.get(`${retirer}`);
        if (response.status === 200) {
          // Transformation des données pour correspondre aux colonnes
          const res = response.data.flatMap(bachelier =>
            bachelier.demandes.map(demande => {
    ...
    async atraiterPayer() {
      ...
      this.error = '';
      try {
        const response = await axios.get(`${atraiter}`);
        if (response.status === 200) {
          // Transformation des données pour correspondre aux colonnes
          const res = response.data.flatMap(bachelier =>
            bachelier.demandes.map(demande => {
    ...
    async disponiblePayer() {
      ...
      this.error = '';
      try {
        const response = await axios.get(`${disponible}`);
        if (response.status === 200) {
          // Transformation des données pour correspondre aux colonnes
          const res = response.data.flatMap(bachelier =>
            bachelier.demandes.map(demande => {
