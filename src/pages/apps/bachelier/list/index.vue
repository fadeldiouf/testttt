    ...
    <VCard id="annee-list">
      <VCardText class="d-flex align-center flex-wrap gap-4">
        ...
        <div class="d-flex align-center flex-wrap gap-6">
          <!-- 👉 Create annee -->
          <VBtn :to="{ name: 'apps-bachelier-add' }">
            Create bachelier
          </VBtn>
        </div>
      </VCardText>
...
import { useBachelierStore } from '@/views/apps/bachelier/store'

const bachelierStore = useBachelierStore()
const {loading,columns,dataListeBachelier } = storeToRefs(bachelierStore);
const { all, destroy } = bachelierStore;
onMounted(()=>{
  all();
});
</script>
