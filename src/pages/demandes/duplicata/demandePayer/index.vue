<template>
  <section>
<p class="text-h6">Duplicatas en cours de traitement</p>
    ...
    <VCard id="annee-list">
      ...
      <v-card >
        ...
        <v-card-text>Voulez vous déplacer la liste selectionnée dans les demandes à traiter?</v-card-text>
        <v-card-actions>
...
import { useDuplicataStore  } from '@/views/demandes/duplicata/store'
import { toast } from 'vue3-toastify';

const duplicataStore = useDuplicataStore()
const {loading,columns,dataListeBachelier,etatCouleurs } = storeToRefs(duplicataStore);
const { encoursPayer, destroym,moveToAtraiter } = duplicataStore;
const confirmDialog = ref(false);
const selectedRows = ref([]); // Variable pour stocker les lignes sélectionnées
...
const confirmAction = async () => {
  try {
    ...
    await moveToAtraiter(selectedRows.value);
    toast.success("Demandes déplacés avec succes !", {
    position: toast.POSITION.TOP_CENTER,
  });
    router.push({ name: 'demandes-duplicata-demande-ensignature'} )
